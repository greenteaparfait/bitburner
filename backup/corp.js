...
export async function main(ns) {
	...
	while (true) {
        ...
        if (ns.getPlayer().money >= ns.getUnlockCost(warehouseUpgrades) )
        {
            ns.purchaseUnlock(warehouseUpgrades);
        };
...
function upgradeWarehouses(ns, division) {
	for (const city of cities) {
		...
		if ( ns.corporation.hasWarehouse(division.name, city) == true) {
...
function newProduct(ns, division) {
	...
	for (var product of division.products) {
		//ns.tprint("line 254: product of division.products = " + product);
		if (ns.corporation.getProduct(division.name, product).developmentProgress < 100) {
			...
			//ns.tprint("line 261: productNumbers = " + productNumbers);
			//ns.tprint("line 262: product = " + product);
			//ns.tprint("line 263: product.length = " + product.length);
			//ns.tprint("line 264: product.charAt(product.length -1) = " + product.charAt(product.length - 1));
			//ns.tprint("line 265: product numbers = " + productNumbers);
	...
	//ns.tprint("line 295: division.products = " + division.products)
	if (productNumbers.length > 0) {
		//ns.tprint("line 296: productNubmers = " + productNumbers);
		newProductNumber = parseInt(productNumbers[productNumbers.length - 1]) + 1;
		//ns.tprint("line 297: newProductNumber = " + newProductNumber);
	...
	//ns.tprint("line 304: new product name = " + newProductName);
