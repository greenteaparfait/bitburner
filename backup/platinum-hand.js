...
export async function main(ns) {
	...
	function takeLongTendies(stock) {
		...
		if (stock.forecast > 0.5) {   // If a stock price is expected to rise,
			...
			ns.print(`INFO\t ${stock.summary} LONG ${ns.formatNumber(curValue, 2)} ${roi}%`);
			...
			stock.shares = 0;
			ns.print(`WARN\t${stock.summary} SOLD for ${ns.formatNumber(saleProfit, 2, 1000, true)} profit`);
		}
	}
			...
			if (stock.forecast > 0.55) {
				if (money > riskThresh) {
					...
					if (ns.stock.buyStock(stock.sym, sharesToBuy) > 0) {
						ns.print(`WARN\t${stock.summary}\t- LONG @ ${ns.formatNumber(sharesToBuy, 2, 1000, true)}`);
	...
	while (true) {
		...
		yolo(stocks);
		// Print ovarall value
		ns.print("Stock value: " + ns.formatNumber(overallValue, 2));
		ns.print("");
		overallValue = 0;
